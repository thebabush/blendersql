...
> `render` does a single still. For an *animation* render (a video, or a frame range), or to render through a specific camera, there's no verb yet — drop to `bpy_exec` (`bpy.ops.render.render(animation=True)` / `write_still=True`), but **one render per `bpy_exec` call**, not a loop: headless Blender wedges after a handful of back-to-back `render.render` calls in one execution (see the `python` skill's gotcha). For many renders, shell out one `blender --background … --python` per render instead. Also: writing a still (PNG etc.) requires `scene.render.image_settings.media_type == 'IMAGE'` — if the scene's output is set to video it's `'VIDEO'` and `file_format` only offers `'FFMPEG'`; flip `media_type='IMAGE'` first, restore after (`python` gotcha has the details).
